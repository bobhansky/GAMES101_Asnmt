    ...
    const std::array<int, 3>& dirIsNeg) const
{
    ...
    // if the ray dirction is negative, then exchange the t min and t max
    if (!dirIsNeg[0]) {
        float temp = tmin_x;
        tmin_x = tmax_x;
        tmax_x = temp;
    ...
    // 这句提示其实非常关键！老师上课的时候经常提到，图形学很少关心相等的时候的条件，
    // 但是这里确实会影响到输出的结果。假如 t_enter = t_exit 时返回的是false，那么作业6并不会出任何问题，
    // 但作业7中会导致只能渲染出左侧的盒子和墙体。因此，如果你在作业6中写的是t_enter < t_exit return true，
    // 请务必改成小于等于。
